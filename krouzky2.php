...
<body>
    ...
    <main class="kr-main">
        <h2>Kroužky druhého stupně</h2>
        <?php 
    $json_data = file_get_contents('./data/krouzky2/krouzky2.json');
    ...
    foreach (array_reverse($json_data) as $index => $item) {
        echo '<div class="ac-item" class="kr-width" onclick="showKr('.$index.', event);">';
            echo '<div class="kr-item-visible">';
                $title = htmlspecialchars($item['title42']);
                $content = htmlspecialchars($item['content42']);
                $image = $item['image42'];
